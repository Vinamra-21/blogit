...
export const postsRouter = router({
  ...
  getByCategory: publicProcedure.input(z.string()).query(async ({ input }) => {
    ...
    const postCats = await db
      ...
      .where(and(eq(postCategories.categoryId, category[0].id), eq(posts.published, true)))
  ...
  delete: protectedProcedure.input(z.number()).mutation(async ({ input, ctx }) => {
    ...
    // Check if user owns the post
    const post = await db.select().from(posts).where(eq(posts.id, input)).limit(1)
  ...
  publish: protectedProcedure.input(z.number()).mutation(async ({ input, ctx }) => {
    const db = getDb()
    ...
    if (post[0].authorId !== ctx.session.userId) {
      throw new TRPCError({ code: "FORBIDDEN", message: "You can only publish your own posts" })
    }

    const updated = await db.update(posts).set({ published: true }).where(eq(posts.id, input)).returning()
    return updated[0]
  }),
